    ...
    def parse(self, testbed, section, steps):
        ...
        for device in testbed:
            ...
            with steps.start('Store data',continue_=True) as step:              
                print(self.parsed_show_bgp_sessions)
                if self.parsed_show_bgp_sessions is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_bgp_sessions_template.render(to_parse_bgp=self.parsed_show_bgp_sessions,device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_BGP_Sessions/%s_show_bgp_sessions.%s" % (device.alias,filetype), "w") as fh:
