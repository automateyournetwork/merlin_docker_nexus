    ...
    def parse(self, testbed, section, steps):
        ...
        for device in testbed:
            ...
            with steps.start('Store data',continue_=True) as step:              
                print(self.parsed_show_ip_route)
                if self.parsed_show_ip_route is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_ip_route_template.render(to_parse_ip_route=self.parsed_show_ip_route['vrf'],device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_IP_Route/%s_show_ip_route.%s" % (device.alias,filetype), "w") as fh:
