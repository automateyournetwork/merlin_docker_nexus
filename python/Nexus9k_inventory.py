            ...
            with steps.start('Store data',continue_=True) as step:              
                if self.parsed_show_inventory is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_inventory_template.render(to_parse_inventory_name=self.parsed_show_inventory['name'],device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_Inventory/%s_show_inventory.%s" % (device.alias,filetype), "w") as fh:
