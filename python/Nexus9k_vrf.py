            ...
            with steps.start('Store data',continue_=True) as step:              
                ...
                if self.parsed_show_vrf is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_vrf_template.render(to_parse_vrf=self.parsed_show_vrf['vrfs'],device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_VRF/%s_show_vrf.%s" % (device.alias,filetype), "w") as fh:
