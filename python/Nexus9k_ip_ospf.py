    ...
    def parse(self, testbed, section, steps):
        ...
        for device in testbed:
            ...
            with steps.start('Store data',continue_=True) as step:              
                print(self.parsed_show_ip_ospf)
                if self.parsed_show_ip_ospf is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_ip_ospf_template.render(to_parse_ip_ospf=self.parsed_show_ip_ospf['vrf'],device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_IP_OSPF/%s_show_ip_ospf.%s" % (device.alias,filetype), "w") as fh:
