    ...
    def parse(self, testbed, section, steps):
        ...
        for device in testbed:
            ...
            with steps.start('Store data',continue_=True) as step:              
                print(self.parsed_show_mac_address_table)
                if self.parsed_show_mac_address_table is not None:
                    ...
                    for filetype in filetype_loop:
                        parsed_output_type = sh_mac_address_table_template.render(to_parse_mac_address_table=self.parsed_show_mac_address_table['mac_table'],device_alias = device.alias,filetype_loop_jinja2=filetype)

                        with open("Camelot/Show_MAC_Address_Table/%s_show_mac_address_table.%s" % (device.alias,filetype), "w") as fh:
